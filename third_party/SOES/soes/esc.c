...
/** Write ESCvar.SM[n] data to ESC PDI control register 0x807(+ offset to
 * SyncManager n).
 *
...
/** Write 0 to Bit0 in SM PDI control register 0x807(+ offset to SyncManager n)
 * to Activate the Sync Manager n.
 *
 * @param[in] n   = Write to Sync Manager no. n
 */
...
/** Write 1 to Bit0 in SM PDI control register 0x807(+ offset to SyncManager n)
 * to De-activte the Sync Manager n.
 *
 * @param[in] n   = Write to Sync Manager no. n
 */
...
/** Read Watchdog Status register 0x440. Result Bit0 0= Expired, 1= Active or
 * disabled.
 *
...
uint16_t ESC_checkDC(void) {
  ...
  if ((sync_act & (ESCREG_SYNC_ACT_ACTIVATED | ESCREG_SYNC_AUTO_ACTIVATED)) >
      0) {
...
/** Check mailbox status by reading all SyncManager 0 and 1 data. The read
 * values are compared with local definitions for SM Physical Address, SM Length
 * and SM Control. If we check fails we disable Mailboxes by disabling
 * SyncManager 0 and 1 and return state Init with Error flag set.
 *
 * @param[in] state   = Current state request read from ALControl 0x0120
 * @return if all Mailbox values is correct we return incoming state request,
 * otherwise we return state Init with Error flag set.
 */
...
/** Try to start mailboxes for current ALControl state request by enabling
 * SyncManager 0 and 1. If all mailbox settings is correct we return incoming
 * state request, otherwise we return state Init with Error flag set and update
 * local ALerror with code 0x16 Invalid mailbox configuration.
 *
 * @param[in] state   = Current state request read from ALControl 0x0120
...
/** Try to start bootstrap mailboxes for current ALControl state request by
 * enabling SyncManager 0 and 1. If all mailbox settings is correct we return
 * incoming state request, otherwise we return state Init with Error flag set
 * and update local ALerror with code 0x16 Invalid mailbox configuration.
 *
 * @param[in] state   = Current state request read from ALControl 0x0120
 * @return if all Mailbox values is correct we return incoming state, otherwise
 * we return state Init with Error flag set.
...
/** Stop mailboxes by disabling SyncManager 0 and 1. Clear local mailbox
 * variables stored in ESCvar.
 */
...
/** Allocate and prepare a mailbox buffer. Take the first Idle buffer from the
 * End. Set Mailbox control state to be used for outbox and fill the mailbox
 * buffer with address master and mailbox next CNT value between 1-7.
 *
 * @return The index of Mailbox buffer prepared for outbox. IF no buffer is
 * available return 0.
 */
...
/** Allocate and prepare a mailbox buffer for sending an error message. Take the
 * first Idle buffer from the end. Set Mailbox control state to be used for
 * outbox and fill the mailbox buffer with error information.
 *
 ...
 * used by Application Layers running on-top of mailboxes. It takes care of
 * sending a mailbox, re-sending a mailbox, reading a mailbox and handles a
 * mailbox full event.
 *
 * @return =0 if nothing to do. =1 if something to be handled by mailbox
 * protocols.
 */
 ...
 * @return = incoming state request if every thing checks out OK. = state (PREOP
 * | ERROR)  if something isn't correct.
 */
...
/** Function trying to enable start updating the process data inputs. It calls
 * the check SM 2 & 3 routine, based on the result from there if enables or
 * disables the Input SyncManager, in addition it updates the ALStatusCode case
 * something didn't pass the check.
 *
 * @param[in] state   = Requested state.
 * @return = state, incoming state request if every thing checks out OK. =state
 * (PREOP | ERROR) if something isn't correct.
 */
