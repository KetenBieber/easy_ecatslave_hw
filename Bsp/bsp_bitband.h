...
/*
    ...
    AliasAddr = BITBAND_BASE + (A - PERIPH_BASE)*8*4 + n*4
        ...
        n*4:将位序号转换为
   外设位带别名区中的字节偏移量，每个位在位带别名区中占用4个字节，所以乘以4
 */
/**
 * @brief "位带地址+位序号"转换"别名区地址"
 */
#define BITBAND(addr, bitnum)                                      \
  ((addr & 0xF0000000) + 0x02000000 + ((addr & 0x000FFFFF) << 5) + \
   (bitnum << 2))
...
#define PBToggle(n) (PBout(n) ^= 1)
#define PCToggle(n) (PCout(n) ^= 1)
#define PDToggle(n) (PDout(n) ^= 1)
#define PEToggle(n) (PEout(n) ^= 1)
