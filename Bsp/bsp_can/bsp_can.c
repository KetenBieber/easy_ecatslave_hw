...
void HAL_CAN_MspInit(CAN_HandleTypeDef* canHandle) {
  ...
  if (canHandle->Instance == CAN0_Instance) {
    ...
    __HAL_CAN_ENABLE_IT(&CAN0_Handle, CAN_IT_BUSOFF | CAN_IT_ERROR);
    ...
    __HAL_CAN_ENABLE_IT(&CAN1_Handle, CAN_IT_BUSOFF | CAN_IT_ERROR);
  }
#endif
}
...
__weak void CAN_Transmit_StdId(CAN_HandleTypeDef* hcan,
  ...
  if (HAL_CAN_AddTxMessage(hcan, &TxHeader, data, &tx_mailbox) != HAL_OK)
    ;
}
